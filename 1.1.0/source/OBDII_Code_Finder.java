...
public class OBDII_Code_Finder {
	...
	public static void main(String args[]) throws IOException {
		...
		exportFile(headerNames, bodyText);
	...
	@SuppressWarnings("resource")
	public static void exportFile(List<String> hn, List<String> bt) throws IOException {
		
		Scanner userInput = new Scanner(System.in);
		
		List<String> resList = Arrays.asList("y", "n", "yes", "no");
		List<String> lines = new ArrayList<String>();
		
		System.out.print("Would you like to export to a text file?>");
		String res = userInput.next();
		
		while (!(resList.contains(res))) {
		...
		if (res.equalsIgnoreCase("y") || res.equalsIgnoreCase("yes")) {
			...
			for (int i = 0; i < hn.size() - 1; i++) {
				lines.add(hn.get(i) + ": " + bt.get(i) + "\n\n");
			}
			
			System.out.print("\nFile Directory [Must Exist]>");
			String fileDir = userInput.next();
			
			System.out.print("File Name [-- for space]>");
			...
			Path processedDocument = Paths.get(fileDir + fileName);
			Files.write(processedDocument, lines, Charset.forName("UTF-8"));
			
			System.out.println("\nSuccessfully export file to " + path + "\n");
	...
	public static List<String> findCodeSectionWithURL(String obd, String url, List<String> headers) throws IOException {
		...
		try {
			doc = Jsoup.connect(urlForDesc).validateTLSCertificates(false).get();
